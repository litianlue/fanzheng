...
import com.dyl.base_lib.external.OnItemClickListener;
...
public class SwipeItemLayout extends ViewGroup {
    ...
    public OnMItemClickListener mListener;

    public  boolean isAllClose = false;
    public interface OnMItemClickListener {
        void onItemClick();
    }

    public void setItemsClickListener(OnMItemClickListener mListener) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //  Log.w("test","ddddmScrollOffset="+mScrollOffset+" pointView="+"mTouchMode="+mTouchMode);
    ...
    public static class OnSwipeItemTouchListener implements RecyclerView.OnItemTouchListener {
        ...
        private  boolean pointOther = false;
        @Override
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_UP:
                    if(mCaptureItem!=null) {
                        if (mCaptureItem.mScrollOffset == 0) {
                            final float x = ev.getX();
                            final float y = ev.getY();
                            if (Math.abs(x - mLastX) < 1 && Math.abs(y - mLastY) < 1) {
                                if (mCaptureItem != null && mCaptureItem.mListener != null) {
                                    if(!pointOther)
                                    mCaptureItem.mListener.onItemClick();
                                }
                            }
                        }
                    }
