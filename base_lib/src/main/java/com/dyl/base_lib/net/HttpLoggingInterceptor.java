...
public final class HttpLoggingInterceptor implements Interceptor {
  ...
  public interface Logger {
    ...
    void body(Chain chain,String json);
    ...
    Logger DEFAULT = new Logger() {
      ...
      @Override
      public void body(Chain chain, String json) {
  ...
  @Override public Response intercept(Chain chain) throws IOException {
    ...
    if (logHeaders) {
      ...
      if (!logBody || !hasRequestBody) {
        ...
        if (isPlaintext(buffer)) {
          logger.body(chain, buffer.readString(charset));
          logger.log(chain, "--> END " + request.method()
    ...
    if (logHeaders) {
      ...
      if (!logBody || !HttpHeaders.hasBody(response)) {
        ...
        if (contentLength != 0) {
          logger.log(chain, "");
          logger.body(chain, buffer.clone().readString(charset));
        }
