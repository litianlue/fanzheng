...
open class MMPRecyclerView : RecyclerView {
    ...
    private var onLoad: (MMPRecyclerView.() -> Unit)? = null
    private var onNext: (MMPRecyclerView.() -> Unit)? = null
        ...
        when (MotionEventCompat.getActionMasked(ev)) {
            MotionEvent.ACTION_MOVE -> {
                ...
                if (activePointerId != INVALID_POINTER) {
                    ...
                    mLastY = y
                    if (hs <= 0) {
                        val h =-deltaY.toInt()// getHeadPy(-deltaY.toInt())
                        moveTo(h)
                        if (!jobs.isActive) {
                            jobs = getJob()
                        }
    ...
    private fun moveTo(move: Int) {
        if (childCount > 0) {
            ...
            if (canMoveTo()) {
                if(move>0){
                    moveTo -= move//moveTo -= move+(computeVerticalScrollOffset()-moveTo)
                }else{
                    moveTo -= move
                }
...
//                invalidate()
    ...
    override fun computeScroll() {
        if (overScroller!!.computeScrollOffset()) {
            if (scrollyY != 0) {
                ...
                var dy = (if (childCount > 0) {
                    if (canMoveTo()) {
                        moveTo
                    } else {
                        computeVerticalScrollOffset()
                    }
                } else totalMove) - overScroller!!.currY
                moveTo(dy)
                invalidate()
            }
    ...
    fun onPullLoad(call: MMPRecyclerView.() -> Unit) {
        onLoad = call
    }

    fun onPullNext(call: MMPRecyclerView.() -> Unit) {
        onNext = call
    }
